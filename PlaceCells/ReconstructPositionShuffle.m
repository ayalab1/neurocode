function [estimations,actual,errors,average] = ReconstructPositionShuffle(positions,spikes,windows,varargin)
...
% See ReconstructPosition for details

nlogdt = sum(bsxfun(@times,spikecount,log(dt)'));
denominator = (sum(logfactorial(spikecount)));
for i=1:max(intervalID)
    ok = intervalID==i & ~uniform;
    shifted = CircularShift(lambda,ceil(rand(1,size(lambda,2))*nBins));
    nloglambda = log(shifted)*spikecount(:,ok);
    numerator = (bsxfun(@plus,nlogdt(ok),nloglambda));
    multiplier = -sum(shifted,2)*dt(ok)';
    Pnx = exp(bsxfun(@minus,numerator+multiplier,denominator(ok)));
    PnxPx = bsxfun(@times,Pnx,Px);
    Pn = nansum(PnxPx);
    estimations(:,ok) = bsxfun(@rdivide,PnxPx,Pn);
end
