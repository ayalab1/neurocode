function varargout = anovabar(data,groups,alpha,compare0,compareGroups)
...
if ~grouped || size(data,2)==1,
    ...
    for i=1:n,
        if ~isempty(x{i}) % depending on the matlab version
        xs(:,i) = nanmean(get(x{i},'xdata'));
        else
            xs(:,i) = handles.bar(i).XEndPoints;
        end
    end
    u = unique(groups)';
    for i=1:length(u);
