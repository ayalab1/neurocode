...
K = 10; 
X = trainNEURAL;
lambdas = sort([logspace(-.5,-4,100)], 'ascend'); 
loadings = [];
regularization = []; 
cost = []; 
...
saveTitle = 'IsolateSequences_Feb22'; 
thisFile = 'C:\Users\emackev\Documents\MATLAB\seqnmf\misc_elm\TestbedFigs_elm.m'; 
papersize = [4 3];
...
readytosave = 0
%% chose lambda=.005; run multiple times, see loadings
loadings = [];
pvals = []; 
is_significant = []; 
X = trainNEURAL;
...
rng(235); %
X = trainNEURAL;
[W, H, cost,loadings,power]= seqNMF(X,'K',K,'L',Lneural,...
...
clf; WHPlot(W(indSort,:,:),H(:,tstart:end), [], 0,trainSONG(:,floor(tstart*SONGfs/VIDEOfs):end),0)
...
clf; WHPlot(W(indperm,:,:),H(:,tstart:end), X(indperm,tstart:end), 0,trainSONG(:,floor(tstart*SONGfs/VIDEOfs):end),0)
savedir = 'C:\Users\emackev\Dropbox (MIT)\SeqNMF\Figures';
