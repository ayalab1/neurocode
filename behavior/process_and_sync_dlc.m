...
if ~exist(fullfile(folder, 'digitalIn.events.mat'), 'file')
    ...
    getDigitalIn('all', 'folder', folder);
end
...
if sum(extra_pulses) > 0
    warning('MyComponent:MyWarningID', 'removing %d frames that were shorter than possible', sum(extra_pulses));
end

bazlerTtl(extra_pulses) = [];

% check if first ttl is too close to start of session
if bazlerTtl(1) < 1
...
elseif isempty(bazlerTtl)
    ...
    notes = 'no ttls';
    warning(notes);
elseif basler_intan_diff > 0 && bazlerTtl(1) > 1 % first ttl is over 1 second from start of recording
    bazlerTtl = bazlerTtl(1:size(x, 1));
    notes = [num2str(basler_intan_diff), ' frames missalignment in total in this session, will assume its from the end'];
    warning(notes)
else
