...
%     'hide'        new figures are hidden to speed up batch
...
if isa(bfile,'function_handle') || (isastring(mfile) && isempty(which(mfile))),
	tmp = bfile;
	bfile = mfile;
	mfile = tmp;
end
...
% flip batch file (to run it backwards)
% b.field = flipud(b.field); b.line = flipud(b.line(:));
