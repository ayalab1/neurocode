...
% winnie changed to the current to accommodate dataset with no condition
% group
if size(placeFieldStats.mapStats{1}) ==  [1 1]
    nCond = 1;
end
...
for c = 1:nCond
    ...
    for unit = 1:nUnits
        if UIDs(unit)
            uu = uu+1;
            templatePF{c}(uu,1) = placeFieldStats.mapStats{unit}{c}.x(1);
            templatePF{c}(uu,2) = placeFieldStats.UID(unit);
            templatePF{c}(uu,3) = unit;
        end
    end
end
