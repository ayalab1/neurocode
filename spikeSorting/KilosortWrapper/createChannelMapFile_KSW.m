...
switch (electrode_type)
    ...
    case 'poly3'
        ...
        for a = 1:ngroups %being super lazy and making this map with loops
            ...
            x(1:extrachannels) = 0;
            y(x == 18) = (1:length(find(x == 18))) * -20;
            y(x == 0) = (1:length(find(x == 0))) * -20 - 10 + extrachannels * 20;
            y(x == -18) = (1:length(find(x == -18))) * -20;
            x = x + a * 200;
            xcoords = cat(1, xcoords, x(:));
            ycoords = cat(1, ycoords, y(:));
    ...
    case 'poly5'
        ...
        for a = 1:ngroups %being super lazy and making this map with loops
            ...
            y(x == 2*18) = (1:length(find(x == 2*18))) * -28;
            y(x == 18) = (1:length(find(x == 18))) * -28 - 14;
            y(x == 0) = (1:length(find(x == 0))) * -28;
            y(x == -18) = (1:length(find(x == -18))) * -28 - 14;
            y(x == 2*-18) = (1:length(find(x == 2*-18))) * -28;
...
if isfield(par, 'SpkGrps')
    skip2 = ~ismember([par.AnatGrps.Channels], [par.SpkGrps.Channels]); % finds the indices of the channels that are not part of SpkGrps
