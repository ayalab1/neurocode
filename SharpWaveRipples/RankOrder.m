...
warning('assumes all cells loaded... sorting is by UID')
warning('results will be incorrect if all cells are not loaded')
...
parfor irep = 1:numRep
    ...
    [tmpcorr, ~, tmpcorrTemplate] = compute_rank_correlation(templateType, ...
        rankUnitsSuffled, evtTimes, templateExt, eventIDs);
...
if doPlot
    ...
    fill([xhist; flip(xhist)], ...
        [mean(yhist, 2) + std(yhist')'; mean(yhist, 2) - std(yhist')']/max(mean(yhist, 2)), ...
        1, 'facecolor', [.7, .9, 1], 'linestyle', 'none', 'DisplayName', '')
...
% RANK CORRELATION...
function [corrMean, corrStd, corrEvents] = compute_rank_correlation(templateType,...
    rankUnits, evtTimes, templateExt, eventIDs)
