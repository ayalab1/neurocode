function ripple_HSE = find_rippleHSE_b(basepath, ripples, varargin)
...
addParameter(p, 'save_neuro2',true); %save neuroscope2

parse(p,varargin{:});
interval = p.Results.interval;
save_evt = p.Results.save_evt;
save_fig = p.Results.save_fig;
save_neuro2 = p.Results.save_fig;
...
if save_evt
    createEVT(ripple_HSE.timestamps(:,1), ripple_HSE.peaks, ripple_HSE.timestamps(:,2), 'saveName', 'F');
end

if save_neuro2
    FRHn2.timestamps = ripple_HSE.timestamps;
    FRHn2.peaktimes = ripple_HSE.peaks;
    save([basename '.FRH.events.mat'], 'FRHn2');
end
