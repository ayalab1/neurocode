...
%%
filetarget = split(basepath, filesep);
filetarget = filetarget{end};
...
for jj = 1:length(analogCh)
    ...
    try % correct for different baselines in different subsessions (different rooms)
        ...
        for subsession = 1:size(MergePoints.timestamps, 1)
            start = MergePoints.timestamps(subsession, 1);
            stop = MergePoints.timestamps(subsession, 2);
            in = xt > start & xt < stop;
    ...
    try % define "effectiveness metric" for the off-vs-on signal separation (pulse) for each subsession:
        ...
        for subsession = 1:size(MergePoints.timestamps, 1)
            ...
            stop = MergePoints.timestamps(subsession, 2);
            in = xt > start & xt < stop;
            dd = Smooth(double(d(in)), smooth);
            midpoint = mean([min(dd), max(dd)]);
    ...
    if ~manualThr
        ...
        if ~isempty(sessEpochs)
            for sess_it = 1:size(sessEpochs, 1)
                start = MergePoints.timestamps(sessEpochs(sess_it), 1);
                stop = MergePoints.timestamps(sessEpochs(sess_it), 2);
                hold on
                xline(start, '--g', 'LineWidth', 2);
                xline(stop, '--r', 'LineWidth', 2);
            end
        end
        xlabel('s');
        ylabel('amp');
    ...
    if manualThr && showFig
        ...
        xlabel('s');
        ylabel(['Ch', num2str(analogCh(jj)), ' (au)']);
        if ~isempty(sessEpochs)
            for sess_it = 1:size(sessEpochs, 1)
                start = MergePoints.timestamps(sessEpochs(sess_it), 1);
                stop = MergePoints.timestamps(sessEpochs(sess_it), 2);
