...
addParameter(p, 'brainRegion', {'PFC', 'MEC', 'EC', 'ILA', 'PL'}, @(x) any(iscell(x), iscchar(x)));
...
if isempty(channel)
    ...
    try
        pBand = bandpower(single(lfp), ...
            session.extracellular.srLfp, passband);

        pTot = bandpower(single(lfp), ...
            session.extracellular.srLfp, ...
            [1, (session.extracellular.srLfp / 2) - 1]);
    catch
        for c = 1:size(lfp, 2)
            pBand(c) = bandpower(single(lfp(:, c)), ...
                session.extracellular.srLfp, passband);

            pTot(c) = bandpower(single(lfp(:, c)), ...
                session.extracellular.srLfp, ...
                [1, (session.extracellular.srLfp / 2) - 1]);
        end
    end
