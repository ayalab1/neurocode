%% Plot macroscopic features (cell, boarders)
figure
file = "E13_16F1(13)";
bw = imread(file+'.png');
load(file+'.mat')
...
ylim([0 1000])
axis equal
plot(line_X_down,line_Y_down, '--w', 'linewidth', 1);
plot(line_X_up,line_Y_up, '--w', 'linewidth', 1);
% scatter(pFoot(:,1),pFoot(:,2));
scatter(data_X,data_Y,[],'ow');
ax = gca;
ax.YDir = 'reverse';

%% Preprocessing for plotting micro
...
%% Plot microscopic features (including projections)
figure
file = "E13_16F1(13)";
bw = imread(file+'.png');
load(file+'.mat')
...
ylim([0 1000])
axis equal
for i=1:length(pHead(:,1))
    plot([pHead(i,1),pFoot(i,1)],[pHead(i,2),pFoot(i,2)],'.-w')
end

plot(line_X_down,line_Y_down, '-w', 'linewidth', 1);
plot(line_X_up,line_Y_up, '-w', 'linewidth', 1);
scatter(pHead(:,1),pHead(:,2),'.','r')
scatter(pFoot(:,1),pFoot(:,2),'.','r')
% scatter(pFoot(:,1),pFoot(:,2));
scatter(data_X,data_Y,[],'ow');
ax = gca;
ax.YDir = 'reverse';
