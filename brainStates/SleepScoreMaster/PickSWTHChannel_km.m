function [SleepScoreLFP,PickChannelStats] = PickSWTHChannel_km(basePath,scoretime,SWWeightsName,...
    Notch60Hz,NotchUnder3Hz,NotchHVS,NotchTheta,channels,SWChannels,ThetaChannels,...
...
if sum(SWChannels)>0 && sum(ThetaChannels)>0
    ...
    badchannels = setdiff(channels,goodchannels);
    rejectchannels = union(rejectchannels,badchannels);
end

usechannels = setdiff(channels,rejectchannels);
