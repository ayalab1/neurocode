...
if size(positions, 2) >= 3
    nDimensions = 2;
    type = 'lll';
    nBinsPerDim = repmat(nBinsPerDim, 1, 2);
end
...
for i = 1:size(actual, 1)
    ...
    q = translateMatrix(estimations(:, :, i), ceil(-actual(i, [2, 1]).*nBinsPerDim+nBinsPerDim/2));
    q(q == 0) = nan;
...
if isempty(reference) || isempty(query), indices = [];
    values = [];
    return;
end
if length(reference) == 1, indices = 1;
    values = reference;
    return;
end
